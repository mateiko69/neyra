...
class OpenAIProvider(AIProvider):
    """OpenAI provider using REST API with strict JSON outputs.

    Uses `AI_MODEL`, `AI_MAX_TOKENS`, `AI_TEMPERATURE`.
    """
    ...
    def analyze_profile(self, profile):
        return {"provider": "openai", "note": "Use /ai/profile-analysis and wingman endpoints."}

    async def generate_openers(self, me, other, *, locale: str | None = None) -> dict:
        loc_key = normalize_ai_request_locale(locale)
        payload = {
            "me": _profile_summary(me),
            "target": _profile_summary(other),
            "locale": loc_key,
        }
        ...
        user = f"ME_PROFILE:\n{payload['me']}\n\nTARGET_PROFILE:\n{payload['target']}\n"
        out = await self._responses_json(system, user, OpenersOut)
    ...
    async def opener_suggestions(
        ...
        *,
        ...
        out = await self._responses_json(system, user, AssistOpenersOut)
    ...
    async def dating_coach_guidance(self, messages: list[str], *, locale: str | None = None) -> dict:
        prompt = _inject_locale_guard(load_prompt("dating_coach_prompt.txt"), locale)
        ctx = "\n".join([sanitize_user_text(m, 240) for m in (messages or [])][-12:])
        out = await self._responses_json(prompt, f"MESSAGES:\n{ctx}\n", DatingCoachOut)
        return _post_filter_coach(out.model_dump(), locale)

    async def suggest_next_step(self, analysis: dict) -> dict:
        prompt = load_prompt("next_step_prompt.txt")
        out = await self._responses_json(prompt, f"ANALYSIS_JSON:\n{analysis}\n", NextStepOut)
        return out.model_dump()

    async def _responses_json(self, system_prompt: str, user_prompt: str, out_model) -> Any:
        ...
        payload = {
            ...
            "max_output_tokens": int(settings.AI_MAX_TOKENS),
            # Ask for JSON in the content; we still parse defensively.
        }
...
def _post_filter_coach(data: dict, locale: str | None) -> dict:
