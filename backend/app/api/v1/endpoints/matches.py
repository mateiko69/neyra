...
def _last_message_preview(db: Session, me: int, partner: int) -> tuple[str | None, str | None]:
    last = (
        db.query(Message)
        .filter(
            or_(
...
def list_matches(limit: int = 50, offset: int = 0, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    for row in rows:
        ...
        preview, preview_at = _last_message_preview(db, int(current_user.id), int(partner_id))
        result.append({
            "match_id": row.id,
