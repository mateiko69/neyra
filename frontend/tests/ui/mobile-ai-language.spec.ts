...
test.describe("Mobile AI language consistency", () => {
  ...
  test("selector + locale consistency + demo language echo", async ({ page }) => {
    async function openAndGenerateSuggestions() {
      ...
      const aiBarGenerate = page.locator('[data-testid="ai-suggestions"] button').first();
      const copilotGenerate = page.locator(".chat-ai-replies button.btn.btn-primary").first();
      if (await aiBarGenerate.isVisible().catch(() => false)) {
        await aiBarGenerate.click();
      } else if (await copilotGenerate.isVisible().catch(() => false)) {
        await copilotGenerate.click();
      }
      const suggestions = page.locator(".chat-ai__suggestion, .chat-ai-replies__option");
    ...
    let serverMessages: any[] = [
      { id: 1, raw_id: 1, sender_id: partnerId, receiver_id: viewerId, content: "Hello there", created_at: new Date().toISOString() },
    ];
    ...
    await page.route("**/api/v1/**", async (route) => {
      ...
      if (method === "GET" && p.endsWith("/api/v1/auth/me")) {
        await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ id: viewerId, onboarding_required: false }) });
        return;
      }
      ...
      if (method === "POST" && p.endsWith("/api/v1/ai/chat-brain/suggestions")) {
        ...
        const locale = String(body?.language || body?.locale || "en").toLowerCase();
    ...
    await page.addInitScript(() => {
      ...
      localStorage.setItem("neyra_ai_suggestion_locale", "auto");
      localStorage.setItem("neyra:locale", "uk");
    });
    ...
    await page.evaluate(() => localStorage.setItem("neyra_ai_suggestion_locale", "en"));
    ...
    await expect.poll(async () => page.evaluate(() => localStorage.getItem("neyra:locale"))).toBe("uk");
    const firstEn = await enSuggestions.first().innerText();
    await page.waitForTimeout(3200);
    await expect(enSuggestions.first()).toContainText(firstEn.slice(0, 12));
    ...
    await page.evaluate(() => {
      ...
      localStorage.setItem("neyra_ai_suggestion_locale", "es");
    });
    await page.reload({ waitUntil: "domcontentloaded" });
